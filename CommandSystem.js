...
module.exports = class CommandSystem {
  ...
  AddCommands = (...commands) => {
    ...
    this.#StringCommands = this.#Commands
    ...
    commands.forEach((command) => {
      ...
      if (
        ...
        this.#Translations
          ...
          .includes(command.Trigger.toLowerCase())
      ) {
        let trans = this.#Translations.find(
          (t) => t.key.toLowerCase() === command.Trigger.toLowerCase()
        );
          ...
          if (command.Subcommands.length > 0) {
            if (this.#FiltersInheritance) {
              command.Subcommands.map((sc) =>
                sc.Filters.push(...command.Filters)
              );
            }

            this.#ProcessCommands(
              prefix
                ? `${prefix} ${trans.translations[lang]}`
      ...
      if (command.Type === "RegExp") {
        ...
        let flags = command.Trigger.flags
          .split("")
          .sort()
          .join("")
          .replace(/(.)(?=.*\1)/g, "");
        let cmd = prefix
          ? `${prefix} ${command.Trigger.source}`
        ...
        if (command.Subcommands.length > 0) {
          ...
          this.#ProcessCommands(
            prefix
              ? `${prefix} ${new RegExp(cmd, flags).source}`
              : new RegExp(cmd, flags).source,
            ...command.Subcommands
          );
        }
      ...
      if (command.Subcommands.length > 0) {
        ...
        this.#ProcessCommands(
          prefix ? `${prefix} ${command.Trigger}` : command.Trigger,
          ...command.Subcommands
        );
      }
    });
  ...
  #ProcessMessage = async (message) => {
    ...
    if (
      this.#Client.CurrentUser.Id === message.Originator &&
      this.#ProcessOwnMessages
    )
      return;
    ...
    // Check String Second
    if (!cmd)
      cmd = this.#StringCommands
        .sort((a, b) => b.trigger.length - a.trigger.length)
        .find((sc) =>
          message.Content.toLowerCase()
            .trim()
            .startsWith(sc.trigger.toLowerCase().trim())
        );
    ...
    let group = new Group([]);
    if (message.IsGroup)
      group = await this.#Client.Groups.GetGroup(message.Recipient.Id);
